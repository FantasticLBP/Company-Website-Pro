...
<script>
    ...
    function refreshView(page) {
        ...
        $.ajax({
            dataType: "json",
            type: "get",
            url: "controller/productlist.php?type=推荐商品&page="+$page,
                ...
                if (data.code == 200) {
                    ...
                    for (var i = 0; i < datas.length; i++) {
                        $('<div class="col-md-3 col-sm-6 masonry-grid-item">' +
                            ...
                            '<a href="productDetail.php?id='+datas[i].id+'&type=推荐商品" class="overlay small">' +
                            '<i class="fa fa-plus"></i>' +
                            '<span>查看详情</span>' +
                            '</a>' +
                            '</div>' +
                            '<div class="listing-item-body clearfix">' +
                            '<h3 class="title"><a href="productDetail.php?id='+datas[i].id+'&type=推荐商品">' + datas[i].productName + '</a></h3>' +
                            '<p>' + datas[i].unit + '</p>' +
                            '<span class="price">¥' + datas[i].price + '</span>' +
                            '<div class="elements-list pull-right">' +
                            '<a href="productDetail.php?id='+datas[i].id+'&type=推荐商品" class="wishlist" title="wishlist"><i class="fa fa-heart-o"></i></a>' +
                            '<a href="productDetail.php?id='+datas[i].id+'&type=推荐商品"><i class="fa fa-shopping-cart pr-10"></i>加入购物车</a>' +
                            '</div>' +
                            '</div>' +
                            '</div>' +
                            '</div>').appendTo($node);
    ...
    $("#search").click(function(){
        ...
        $.ajax({
            dataType: "json",
            type: "get",
            url: "controller/productlist.php?type=推荐商品&order="+order+"&startPrice="+startPrice+"&endPrice="+endPrice+"&direction="+direction+"&productBigKind="+productBigKind+"&page="+page,
            success: function (data) {
                if (data.code == 200) {
                    var $pagetarget = $(".pagination");
                    var datas = data.data.datas;
