...
class FAQService
{
    ...
    function operateService()
    {
        ...
        if($this->type === "select"){
            ...
            $time = date("Y年m月d日");
            $data = ["username"=>$this->username,"kind"=>$this->kind,"email"=>$this->email,"content" => $this->content,"time"=>$time];
            $Res = $mysqlPdo->add($data, $this->tableName);

            $lastInsertId = $mysqlPdo->getLastInsertId();
            if ($Res){
                echo '<script>window.location.href = "../FAQ.php?m=as"</script>';
            } else {
                echo '<script>window.location.href = "../FAQ.php?m=af"</script>';
            }
